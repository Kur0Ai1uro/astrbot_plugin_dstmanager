...
- `/饥荒玩家 张三` 检索进过本服的人；翻页用 `-p` 或末尾数字。名字含空格时加引号
...
| `/饥荒玩家` | 所有人 | 全部历史。翻页：`/饥荒玩家 -p 2` 或 `/饥荒玩家 2` |
| `/饥荒玩家 张三` | 所有人 | 检索。空格或数字本身是名字时加引号：`/饥荒玩家 "张三 2"` |
| `/饥荒玩家 --new "张三" KU_xxxxxxxx` | 所有人 | 补全对照。`/饥荒新玩家` 仍可用 |
| `/饥荒玩家 --delete 张三` | 所有人 | 删除一条对照。清空全部：`/饥荒玩家 --delete -A` |
| `/饥荒物品 金块 -p 2` | 所有人 | 查 prefab 与 `c_give` |
| `/饥荒指令 封禁 -p 2` | 所有人 | 模糊检索控制台用法，不执行 |
| `/饥荒订阅`               | 管理员 | 把进出推送到当前会话                     |
| `/饥荒取消订阅`             | 管理员 | 取消推送                           |
